...
const props = defineProps({
  sources: Array<NavBarsSource>
});

const navigation = (props.sources) ? props.sources : [];

const navigate = (url: string, event: Event) => {
  event.preventDefault();
  const nav = useNavigation().value;
  nav(url);
  return;
}
</script>
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <button v-for="item in navigation"
                      ...
                      @click="navigate(item.href, $event)">{{ item.name }}
              </button>
            </div>
          </div>
        </div>
