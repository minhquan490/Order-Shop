...
<template>
  <div>
    <div class="rounded-xl box-shadow w-full">
      <div>
        <div class="p-4">
          <Icon :name="tableIconName as string" width="28" height="28" />
          ...
          <table class="w-full table">
            ...
            <thead class="border-b">
              <tr class="grid p-4 row" :style="`--cols: ${table.columns}`">
                <th :id="header.dataPropertyName" v-for="header in table.headers" class="col-span-1 border-gray-400 w-3/4">
                  <button class="flex items-center justify-center w-full" @click="sort(header.dataPropertyName, $event)" ref="table-head-button" :disabled="table.source?.length === 0">
                    <span :class="table.source?.length === 0 ? 'text-xs w-max' : 'hover:cursor-pointer text-xs w-max'" v-text="header.name"></span>
                  </button>
                </th>
              </tr>
            </thead>
            <tbody :style="`--height: ${table.height}`" class="body block">
              <template v-if="tableRender">
                <tr v-for="(data, i) in table.data" class="grid p-4 hover:bg-gray-200 border-b row" :style="`--cols: ${table.columns}`" @click="selectRow(i)" ref="table-body">
