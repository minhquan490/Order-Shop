...
export class HttpJsonOpener implements HttpServiceProvider {
    private readonly singleton: InternalHttpService;
...
class InternalHttpService implements HttpService {
    ...
    private readonly requestHeaderStrategy: Strategy<XMLHttpRequest>;
    ...
    get<T, U>(request?: T, urlParams?: Map<string, string>): ResponseWrapper<U | ErrorResponse | null> {
        return this.sendRequestOnClient(HttpMethod.GET, request, urlParams);
    }

    post<T, U>(request: T, urlParams?: Map<string, string>): ResponseWrapper<U | ErrorResponse | null> {
        return this.sendRequestOnClient(HttpMethod.POST, request, urlParams);
    }

    patch<T, U>(request: T, urlParams?: Map<string, string>): ResponseWrapper<U | ErrorResponse | null> {
        return this.sendRequestOnClient(HttpMethod.PATCH, request, urlParams);
    }

    delete<T, U>(request?: T, urlParams?: Map<string, string>): ResponseWrapper<U | ErrorResponse | null> {
        return this.sendRequestOnClient(HttpMethod.DELETE, request, urlParams);
    }

    private sendRequestOnClient<T, U>(method: HttpMethod, request?: T, urlParams?: Map<string, string>): ResponseWrapper<U | ErrorResponse | null> {
        let req = this.requestFactory.getInstance();
        req.timeout = 3000;
        const path = this.createPath(this.url, urlParams);
        req.open(method.toString(), path);
