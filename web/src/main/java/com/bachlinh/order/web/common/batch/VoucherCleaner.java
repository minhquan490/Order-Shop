...
public class VoucherCleaner extends AbstractJob {
    ...
    protected void doExecuteInternal() {
        Where modifiedWhere = Where.builder()
                .attribute(AbstractEntity_.MODIFIED_DATE)
                .value(Timestamp.valueOf(LocalDateTime.now().plusMonths(-REMOVAL_POLICY)))
                .operation(Operation.LT)
                .build();
        Where enabledWhere = Where.builder()
                .attribute(Voucher_.ACTIVE)
                .value(false)
                .operation(Operation.EQ)
                .build();
        Join customerJoin = Join.builder()
                .attribute(Voucher_.CUSTOMERS)
                .type(JoinType.INNER).build();
