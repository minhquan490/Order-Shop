...
public class CriteriaPredicateParser {
    ...
    private Predicate where(Root<?> root, Where where) {
        return switch (where.getOperator()) {
            ...
            case BETWEEN -> throw new UnsupportedOperationException();
        };
    }

    public CriteriaPredicateParser select(Select... select) {
        for (var s : select) {
    ...
    public CriteriaPredicateParser where(Where... where) {
        for (var w : where) {
    ...
    public CriteriaPredicateParser orderBy(OrderBy... order) {
        for (var o : order) {
            switch (o.getType()) {
    ...
    public CriteriaPredicateParser join(Join... join) {
        for (var j : join) {
