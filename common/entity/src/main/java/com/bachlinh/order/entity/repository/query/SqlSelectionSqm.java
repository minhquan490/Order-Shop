...
class SqlSelectionSqm extends AbstractSql<SqlSelect> implements SqlSelect {
    ...
    SqlSelectionSqm(TableMetadataHolder targetMetadata, FormulaMetadata formulaMetadata, Map<Class<? extends AbstractEntity<?>>, TableMetadataHolder> tableMetadata) {
    ...
    public SqlSelect select(Select select, String functionName) {
        ...
        String column = String.format(functionPattern, functionName, targetMetadata.getColumn(select.getColumn()));
        if (select.getAlias() != null) {
            column = String.format(ALIAS_PATTERN, column, select.getAlias());
        }
        this.selects.add(new SelectHolder(select.getColumn(), column));
    ...
    private String selectFromQuery() {
        ...
        if (selects.isEmpty() && otherSelects.isEmpty()) {
            ...
            while (!this.selects.isEmpty()) {
                ...
                String processedCol;
                if (QueryUtils.isQueryStartWithFunction(select, functionDialect)) {
                    processedCol = select;
                } else {
                    processedCol = MessageFormat.format(columnSelectPattern, this.targetMetadata.getTableName(), select);
                }
            ...
            while (!this.otherSelects.isEmpty()) {
                ...
                String processedCol;
                if (QueryUtils.isQueryStartWithFunction(otherSelect.columnName(), functionDialect)) {
                    processedCol = otherSelect.columnName();
                } else {
                    processedCol = MessageFormat.format(columnSelectPattern, otherSelect.tableName(), otherSelect.columnName());
                }
