 ...
 *         channel.pipeline().addLast("idleStateHandler", new {@link io.netty.handler.timeout.IdleStateHandler}(60, 30, 0));
 *         channel.pipeline().addLast("myHandler", new MyHandler());
 *     }
 * }
 *
 * // Handler should handle the {@link IdleStateEvent} triggered by {@link io.netty.handler.timeout.IdleStateHandler}.
...
public class IdleStateHandler extends ChannelDuplexHandler {
    ...
    protected void channelIdle(ChannelHandlerContext ctx, IdleStateEvent evt) {
        ...
        switch (idleState) {
            case IdleState.ALL_IDLE -> ctx.close();
            case IdleState.READER_IDLE -> {/* Do nothing */}
            case IdleState.WRITER_IDLE -> {/* Do nothing */}
        }
    }
    ...
    protected IdleStateEvent newIdleStateEvent(IdleState state, boolean first) {
        switch (state) {
            ...
            default:
                String message = StringTemplate.STR. "Unhandled: state=\{ state }, first=\{ first }" ;
    ...
    /**
     * Returns {@code true} if and only if the {@link io.netty.handler.timeout.IdleStateHandler} was constructed
